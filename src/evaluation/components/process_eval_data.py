...
class ProcessEvalData:
    def __init__(self, df_dict: dict, model_name):
        ...
        self.module_path = self.original_config.get("data_processing_modules_path")
        self.filter_function = self.original_config.get("filter_function")
        self.primary_key = self.original_config.get("primary_key")
        self.feature_eng_list = self.original_config.get("feature_engineering")
        self.target_eng_list = self.original_config.get("target_engineering")
        self.features = self.original_config.get("define_features")
        self.targets = self.original_config.get("define_targets")
        self.category_index = self.original_config.get("category_index")
        self.scaler_save_path = self.original_config.get("scaler_save_path")
        self.reshape = self.original_config.get("reshape")
        
        self.process_data_config = self._load_data_processing_config()
        self.fi = self.process_data_config.get("filter_indices")
        self.e = self.process_data_config.get("engineering")
        self.em = self.process_data_config.get("engineering_mode")
        self.ec = self.process_data_config.get("encode_categtories")
        self.pf = self.process_data_config.get("prepare_features")
        self.pt = self.process_data_config.get("prepare_targets")
    ...
    def _load_data_processing_config(self):
        """
        Dynamically load the original `data_processing_config` module from the 
        requirements directory.

        Returns:
            dict: The `data_processing_config` dictionary from the original file.
        ...
        # Load the module dynamically
        spec = importlib.util.spec_from_file_location("process_data_config", original_config_path)
        config_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(config_module)
        
        # Return the config dictionary
        return config_module.process_data_config
